...
namespace Eura
{
    ...
    diagnostic_workspace_client_capabilities) noexcept -> void
    {
        if(object.contains("refreshSupport"))
            diagnostic_workspace_client_capabilities.refreshSupport = object.at("refreshSupport").
            get<bool>();
    }
    ...
    diagnostic_workspace_client_capabilities) noexcept -> void
    {
        if(diagnostic_workspace_client_capabilities.refreshSupport.has_value())
            object["refreshSupport"] = *diagnostic_workspace_client_capabilities.refreshSupport;
    }
}
