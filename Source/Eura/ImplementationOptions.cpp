...
namespace Eura
{
    ...
    noexcept -> void
    {
        from_json(object, static_cast<WorkDoneProgressOptions&>(implementation_options));
    }

    auto to_json(nlohmann::json& object, const ImplementationOptions& implementation_options)
    noexcept -> void
    {
        to_json(object, static_cast<const WorkDoneProgressOptions&>(implementation_options));
    }
}
