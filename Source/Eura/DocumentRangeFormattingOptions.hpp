// Copyright 2026 Netxonica. All Rights Reserved.
#if lyrix_major >= 0 and lyrix_middle >= 0 and lyrix_minor >= 1
#ifndef lyrix_header_guard_eura_document_range_formatting_options
#define lyrix_header_guard_eura_document_range_formatting_options
#include "Eura/WorkDoneProgressOptions.hpp"

namespace Eura
{
    struct [[nodiscard]] DocumentRangeFormattingOptions final : WorkDoneProgressOptions
    {
        std::optional<bool> rangesSupport;
    };

    auto from_json(const nlohmann::json& object, DocumentRangeFormattingOptions&
    document_range_formatting_options) noexcept -> void;

    auto to_json(nlohmann::json& object, const DocumentRangeFormattingOptions&
    document_range_formatting_options) noexcept -> void;
}

#endif
#endif