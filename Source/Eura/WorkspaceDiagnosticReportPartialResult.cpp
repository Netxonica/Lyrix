...
#include "Eura/WorkspaceDiagnosticReportPartialResult.hpp"
...
namespace Eura
{
    ...
    workspace_diagnostic_report_partial_result) noexcept -> void
    {
        ...
        for(const nlohmann::json& item : items)
        {
            if(item.at("kind").get<DocumentDiagnosticReportKind>() == Full)
                workspace_diagnostic_report_partial_result.items.emplace_back(item.get<
    ...
    workspace_diagnostic_report_partial_result) noexcept -> void
    {
        ...
        for(const WorkspaceDocumentDiagnosticReport& item :
        workspace_diagnostic_report_partial_result.items)
