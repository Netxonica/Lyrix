// Copyright 2026 Netxonica. All Rights Reserved.
#if lyrix_major >= 0 and lyrix_middle >= 0 and lyrix_minor >= 1
#ifndef lyrix_header_guard_eura_stale_request_support_options
#define lyrix_header_guard_eura_stale_request_support_options
#include "Eura/Json.hpp"

namespace Eura
{
    struct [[nodiscard]] StaleRequestSupportOptions final
    {
        bool cancel;

        std::vector<std::string> retryOnContentModified;
    };

    auto from_json(const nlohmann::json& object, StaleRequestSupportOptions&
    stale_request_support_options) noexcept -> void;

    auto to_json(nlohmann::json& object, const StaleRequestSupportOptions&
    stale_request_support_options) noexcept -> void;
}

#endif
#endif