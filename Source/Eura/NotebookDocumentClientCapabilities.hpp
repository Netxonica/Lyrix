// Copyright 2026 Netxonica. All Rights Reserved.
#if lyrix_major >= 0 and lyrix_middle >= 0 and lyrix_minor >= 1
#ifndef lyrix_header_guard_eura_notebook_document_client_capabilities
#define lyrix_header_guard_eura_notebook_document_client_capabilities
#include "Eura/NotebookDocumentSyncClientCapabilities.hpp"

namespace Eura
{
    struct [[nodiscard]] NotebookDocumentClientCapabilities final
    {
        NotebookDocumentSyncClientCapabilities synchronization;
    };

    auto from_json(const nlohmann::json& object, NotebookDocumentClientCapabilities&
    notebook_document_client_capabilities) noexcept -> void;

    auto to_json(nlohmann::json& object, const NotebookDocumentClientCapabilities&
    notebook_document_client_capabilities) noexcept -> void;
}

#endif
#endif