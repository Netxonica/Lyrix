// Copyright 2026 Netxonica. All Rights Reserved.
#if lyrix_major >= 0 and lyrix_middle >= 0 and lyrix_minor >= 1
#include "Eura/LinkedEditingRangeOptions.hpp"

namespace Eura
{
    auto from_json(const nlohmann::json& object, LinkedEditingRangeOptions&
    linked_editing_range_options) noexcept -> void
    {
        from_json(object, static_cast<WorkDoneProgressOptions&>(linked_editing_range_options));
    }

    auto to_json(nlohmann::json& object, const LinkedEditingRangeOptions&
    linked_editing_range_options) noexcept -> void
    {
        to_json(object, static_cast<const WorkDoneProgressOptions&>(linked_editing_range_options));
    }
}

#endif