...
{
    const std::string content = "{\"kind\":\"unchanged\",\"resultId\":\"meow\",\"relatedDocuments\":{\"meow.extension\":{\"kind\":\"full\",\"resultId\":\"meow\",\"items\":[]},\"nya.extension\":{\"kind\":\"unchanged\",\"resultId\":\"meow\"}}}";
    const nlohmann::json root = nlohmann::json::parse(content);
    Eura::RelatedUnchangedDocumentDiagnosticReport request;
    Eura::from_json(root, request);
    if(request.kind not_eq Eura::Unchanged or request.resultId not_eq "meow" or not request.
    relatedDocuments.has_value() or request.relatedDocuments->size() not_eq 2uz)
        return false;
