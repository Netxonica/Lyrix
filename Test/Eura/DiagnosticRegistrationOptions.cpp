...
{
    const std::string content = "{\"documentSelector\":[{\"language\":\"lyrix\",\"scheme\":\"meow.lyrix\",\"pattern\":{\"baseUri\":{\"uri\":\"meow.extension\",\"name\":\"meow nya\"},\"pattern\":\"*.extension\"}},{\"notebook\":{\"notebookType\":\"lyrix\",\"scheme\":\"meow.lyrix\",\"pattern\":{\"baseUri\":{\"uri\":\"meow.extension\",\"name\":\"meow nya\"},\"pattern\":\"*.extension\"}},\"language\":\"lyrix\"}],\"workDoneProgress\":true,\"identifier\":\"meow\",\"interFileDependencies\":true,\"workspaceDiagnostics\":true,\"id\":\"meow\"}";
    const nlohmann::json root = nlohmann::json::parse(content);
    Eura::DiagnosticRegistrationOptions request;
    Eura::from_json(root, request);
    if(not std::holds_alternative<Eura::DocumentSelector>(request.documentSelector))
        return false;
    ...
    request.workDoneProgress.has_value() or not *request.workDoneProgress or not request.identifier
    .has_value() or *request.identifier not_eq "meow" or not request.interFileDependencies or not
    request.workspaceDiagnostics or not request.id.has_value() or *request.id not_eq "meow")
        return false;
