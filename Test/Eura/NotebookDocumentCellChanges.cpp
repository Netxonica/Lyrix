...
{
    const std::string content = "{\"structure\":{\"array\":{\"start\":67,\"deleteCount\":42,\"cells\":[{\"kind\":2,\"document\":\"meow.extension\",\"metadata\":{\"jsonrpc\":\"2.0\"},\"executionSummary\":{\"executionOrder\":67,\"success\":true}}]},\"didOpen\":[{\"uri\":\"meow.extension\",\"languageId\":\"lyrix\",\"version\":67,\"text\":\"meow nya\"}],\"didClose\":[{\"uri\":\"meow.extension\"}]},\"data\":[{\"kind\":2,\"document\":\"meow.extension\",\"metadata\":{\"jsonrpc\":\"2.0\"},\"executionSummary\":{\"executionOrder\":67,\"success\":true}}],\"textContent\":[{\"document\":{\"uri\":\"meow.extension\",\"version\":67},\"changes\":[{\"text\":\"meow meow\"}]}]}";
    const nlohmann::json root = nlohmann::json::parse(content);
    Eura::NotebookDocumentCellChanges request;
    Eura::from_json(root, request);
    if(request.structure->array.start not_eq 67u or request.structure->array.deleteCount not_eq 42u
    or not request.structure->array.cells.has_value() or request.structure->array.cells->size()
    not_eq 1uz)
        return false;
    const Eura::NotebookCell& cell = (*request.structure->array.cells)[0uz];
    ...
    or not *cell.executionSummary->success or not request.structure->didOpen.has_value() or request
    .structure->didOpen->size() not_eq 1uz)
        return false;
    const Eura::TextDocumentItem item = (*request.structure->didOpen)[0uz];
    if(item.uri not_eq "meow.extension" or item.languageId not_eq "lyrix" or item.version not_eq 67
    or item.text not_eq "meow nya" or not request.structure->didClose.has_value() or request.
    structure->didClose->size() not_eq 1uz or (*request.structure->didClose)[0uz].uri not_eq
    "meow.extension" or not request.data.has_value() or request.data->size() not_eq 1uz)
        return false;
    const Eura::NotebookCell& cell2 = (*request.data)[0uz];
    ...
    or not *cell2.executionSummary->success or not request.textContent.has_value() or request.
    textContent->size() not_eq 1uz)
        return false;
    const Eura::NotebookDocumentCellContentChanges& changes = (*request.textContent)[0uz];
